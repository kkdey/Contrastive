...
class CPCA(object):
    ...
    def fit_transform(self, foreground, lst, plot=False, gui=False, alpha_selection='auto', n_alphas=40,  max_log_alpha=3, n_alphas_to_return=4, active_labels = None, colors=None, legend=None, alpha_value=None, return_alphas=False):
        self.fit(foreground, lst)
        return self.transform(self.fg , lst , alpha_selection=alpha_selection,  n_alphas=n_alphas, max_log_alpha=max_log_alpha, n_alphas_to_return=n_alphas_to_return, plot=plot, gui=gui, active_labels=active_labels, colors=colors, legend=legend, alpha_value=alpha_value, return_alphas=return_alphas)
    ...
    def fit(self, foreground, lst, preprocess_with_pca_dim=None):
        # Housekeeping
        ...
        self.bg_cov = []
        for y in lst:
            y = y.T.dot(y)/(y.shape[0]-1)
            self.bg_cov.append(y)
    ...
    def transform(self, dataset , lst , alpha_selection='auto', n_alphas=40, max_log_alpha=3, n_alphas_to_return=4, plot=False, gui=False, active_labels = None, colors=None, legend=None, alpha_value=None, return_alphas=False):
    ...
    def cpca_alpha(self, dataset, alpha=1):
        ...
        min_sigma = self.fg_cov - alpha*self.bg_cov[0]
        min_l2 = np.linalg.norm(min_sigma)
        for y in self.bg_cov[1:]:
            
            sigma = self.fg_cov - alpha*y
            l2 = np.linalg.norm(sigma)
            if(l2 < min_l2):
                min_l2 = l2
                min_sigma = sigma
        

        w, v = LA.eig(min_sigma)
    ...
    def cpca_multi(self, lst , n_alphas = 40, max_log_alpha = 3, n_alphas_to_return =4):
        ...
        for x in lst:
            ...
            projected_data, alpha1 , v_top = self.fit_transform(self.fg,lst,return_alphas=True)
            lst.insert(i,x)
    ...
    # def help1(self,x,lst,n_alphas, max_log_alpha, n_alphas_to_return):
    #     best_alphas, all_alphas, _, _ = self.find_spectral_alphas(n_alphas, max_log_alpha, n_alphas_to_return)
    #     alpha = best_alphas[0]
    #     i = 0
    #     min_i = 0
    #     cov_x = x.T.dot(x)/(x.shape[0]-1)
    #     cov_y = lst[0].T.dot(lst[0])/(lst[0].shape[0]-1)
    #     min_sigma = cov_x - alpha*cov_y
    #     for y in lst[1:]:
    #         cov_y = y.T.dot(y)/(y.shape[0]-1)
    #         sigma = cov_x - alpha*cov_y
    #         if(sigma < min_sigma):
    #             min_i = i 
    #             min_sigma = sigma
